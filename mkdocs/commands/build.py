...
def _populate_page(page, config, files, dirty=False):
    ...
    try:
        ...
        page.content = config['plugins'].run_event(
            'page_content', page.content, page=page, config=config, files=files, dirty=dirty
        )
    except Exception as e:
...
def _build_page(page, config, files, nav, env, dirty=False):
    ...
    try:
        ...
        context = config['plugins'].run_event(
            'page_context', context, page=page, config=config, nav=nav, dirty=dirty
        )
        ...
        output = config['plugins'].run_event(
            'post_page', output, page=page, config=config, dirty=dirty
        )
...
def build(config, live_server=False, dirty=False):
    ...
    files = config['plugins'].run_event('files', files, config=config, dirty=dirty)

    nav = get_navigation(files, config)

    # Run `nav` plugin events.
    nav = config['plugins'].run_event('nav', nav, config=config, files=files, dirty=dirty)
    ...
    env = config['plugins'].run_event(
        'env', env, config=config, files=files, dirty=dirty
    )
